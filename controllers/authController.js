...
exports.login = async (req, res) => {
  try {
    ...
    // Device/session limit logic
    // For Free plan, deviceLimit = 1; for paid, use user's deviceLimit field
    const deviceLimit = user.subscriptionStatus === "active"
      ? (user.deviceLimit || 1)
      : 1;
    ...
    if (activeSessions >= deviceLimit) {
      return res.status(403).json({
        error: `Maximum device limit reached (${deviceLimit}). Please logout from another device or upgrade your plan.`
      });
    }
